...
static inline int GifIMin(int l, int r) { return l < r ? l : r; }
static inline int GifIAbs(int i) { return i < 0 ? -i : i; }

// walks the k-d tree to pick the palette entry for a desired color.
// Takes as in/out parameters the current best color and its error -
// only changes them if it finds a better color in its subtree.
// this is the major hotspot in the code at the moment.
static void GifGetClosestPaletteColor(GifPalette* pPal, int r, int g, int b,
...
static int GifPartition(uint8_t* image, const int left, const int right,
...
// median
static void GifPartitionByMedian(uint8_t* image, int left, int right, int com,
                                 int neededCenter) {
  if (left < right - 1) {
...
static void GifSplitPalette(uint8_t* image, int numPixels, int firstElt,
...
// changed pixels only.
static int GifPickChangedPixels(const uint8_t* lastFrame, uint8_t* frame,
                                int numPixels) {
  int numChanged = 0;
...
static void GifDitherImage(const uint8_t* lastFrame, const uint8_t* nextFrame,
...
static void GifThresholdImage(const uint8_t* lastFrame,
...
static void GifWriteCode(FILE* f, GifBitStatus* stat, uint32_t code,
                         uint32_t length) {
